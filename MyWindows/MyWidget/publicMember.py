...
class PublicMember:
    ...
    def __init__(self):
        ...
        self.Y0 = []
        self.Z0 = []
        self.xyzDf = None  # idw插值后的分维值及其坐标数据框
    ...
    def saveFile(self):
        ...
        try:
            with asksaveasfile(title='另存为', initialfile='新建文件', defaultextension='',
                ...
                self.xyzDf.to_excel(f.name, sheet_name='Sheet1', index=False)
        except TypeError:
            pass
    ...
    def image_init(self, my_ana_dict: dict, radius, n):
        ...
        self.xyzDf = pd.DataFrame({'x': self.X0.reshape(len(self.X0) * len(self.X0[0])),
                                   'y': self.Y0.reshape(len(self.Y0) * len(self.Y0[0])),
                                   'z': z0})
        return len(z0)
